...
int main() {
  ...
  client.dict_get_content(lang, "Nice", [&](auto content) {
    std::string desc = std::string(content[0].begin(), content[0].end());
    std::cout << "got: \n-------------\n" << desc << "\n-----------" << std::endl;

    if (desc != "Pronunciation") {
      if (desc.find("A coastal city,") == std::string::npos) {
        exit(1);
      }
      if (desc.find("Proper noun") == std::string::npos) {
        exit(2);
      }
      if (desc.find("A surname.") == std::string::npos) {
        exit(3);
      }
    }
